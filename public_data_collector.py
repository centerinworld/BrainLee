...
LEND_BASE   = "https://apis.data.go.kr/1160100/service/GetStocLendBorrInfoService"
...
def _get_keys():
    try:
        ...
        dart_key = getattr(config, "DART_API_KEY2", os.getenv("DART_API_KEY2",  # 공시정보 → KEY2
                   getattr(config, "DART_API_KEY", os.getenv("DART_API_KEY", ""))))
        return pub_key, dart_key
    except Exception:
        return os.getenv("PUBLIC_DATA_API_KEY", ""), os.getenv("DART_API_KEY2", os.getenv("DART_API_KEY", ""))
...
def _api_call(url: str, params: dict, max_rows: int = 5000) -> list:
    ...
    while True:
        ...
        try:
            ...
            logger.warning(f"API 호출 오류: {url} - {type(e).__name__}")
            break
...
def collect_short_sell(conn, bas_dt: str) -> int:
    ...
    saved = 0
    for it in items:
        isin = str(it.get("isinCd", "")).strip()
        # 한국 종목만 (KR로 시작하는 12자리 ISIN)
        if not isin.startswith("KR") or len(isin) != 12:
            continue
        # ISIN → 6자리 종목코드 (KR7XXXXXXX1 구조에서 [3:9])
        code = isin[3:9]
        if not code.isdigit():
            continue
        bal_qty = _fval(it.get("lnbRmanStckCnt"))  # 대차잔고주수
        try:
            conn.execute("""
            ...
            """, (
                bas_dt, code,
                it.get("isinCdNm", ""),
                _fval(it.get("lnbCclStckCnt")),   # 대차체결주수
                None,                              # 금액 없음
                bal_qty,                           # 대차잔고주수
                None,                              # 잔고금액 없음
                None,                              # 비율 별도 계산 필요
...
def collect_investor_trading(conn, bas_dt: str) -> int:
    ...
    2026-08-14: getInvstByTrdrStkInfo는 공공데이터포털에서 폐지된 서비스(CLAUDE.md
    섹션9 기존 확인사항) — 로그 전체 이력(2026-06-24~08-14, 2개월+)에서 단 1건도
    저장된 적이 없음을 실측 확인. 매일 왕복 네트워크 호출+대기(_api_call 재시도)만
    낭비하고 있어 즉시 스킵. 투자자 수급은 kiwoom_investor_daily(정상 동작)가 이미
    대체 중이므로 기능 손실 없음. 만약 서비스가 부활하면 아래 return 0 줄만 지우면 됨.
    """
    logger.debug(f"[투자자] {bas_dt} 스킵(getInvstByTrdrStkInfo 폐지된 API, kiwoom_investor_daily로 대체됨)")
    return 0
...
def collect_foreign_holding(conn, bas_dt: str) -> int:
    ...
    2026-08-25: getFrgnIsttTrdrStkInfo가 2026-06-08 이후 매일 0건만 반환하는
    것을 실측 확인(collect_investor_trading과 동일 클래스의 죽은 API) — 매일
    03:00(cron_3am.sh)마다 헛되이 API 왕복만 반복 중이었음. 외국인 보유율은
    kiwoom_foreign_flow(ka10008, 2340종목·더 신선)가 이미 대체 중이므로 스킵해도
    기능 손실 없음. 서비스가 부활하면 아래 return 0 줄만 지우면 됨.
    """
    logger.debug(f"[외국인] {bas_dt} 스킵(getFrgnIsttTrdrStkInfo 죽은 API, kiwoom_foreign_flow로 대체됨)")
    return 0
    ...
    if not items:
        logger.debug(f"[외국인] {bas_dt} 미신청 서비스")
        return 0
...
def collect_listed_company(conn, bas_dt: str) -> int:
    ...
    2026-08-25: GetKrxListedInfoService/getItemInfo가 매주 HTTPError만 반환하는
    것을 실측 확인(listed_company_info는 애초에 0행) — 매주 월요일 07:00마다
    헛되이 API 왕복만 반복 중이었음. 서비스가 복구되면 아래 return 0 줄만 지우면 됨.
    """
    logger.debug(f"[기업정보] {bas_dt} 스킵(GetKrxListedInfoService 죽은 API, 실측 0행 확인)")
    return 0
    ...
    if args.backfill:
        ...
        while cur <= end:
            if cur.weekday() < 5:  # 평일만
                collect_all_for_date(conn, cur.strftime("%Y%m%d"))
                time.sleep(1.0)
