...
source ~/.zshrc && cd /Applications/stock_dashboard && source venv/bin/activate
...
python3 /Applications/stock_dashboard/collect_naver_investor.py --years 5 \
...
python3 /Applications/stock_dashboard/dart_backfill_2024_2025.py \
...
# [4] 대차잔고 5년치 (순차)
echo "[4/6] 대차잔고 5년치..."
python3 /Applications/stock_dashboard/collect_short_5years.py --mode all \
  >> "$LOG_DIR/short_5y_${TS}.log" 2>&1
echo "  완료 (종료코드: $?) | log: $LOG_DIR/short_5y_${TS}.log"

# [5] KIS OHLCV 최근 7일 갱신 (누락 보완)
echo "[5/6] KIS OHLCV 최근 7일 갱신..."
python3 /Applications/stock_dashboard/collect_kis_ohlcv.py --days 7 \
  >> "$LOG_DIR/kis_ohlcv_7d_${TS}.log" 2>&1
echo "  완료 (종료코드: $?)"

# [6] NPS V2 고용 가용 과거월 백필
# from-ym=201901로 요청하되, API가 실제 노출하는 월별 seq/dataCrtYm 범위만 저장된다.
echo "[6/6] NPS V2 고용 가용 과거월 백필..."
python3 /Applications/stock_dashboard/employment_monitor/collect_nps_monthly.py \
  --historical --from-ym 201901 --months-back 0 \
