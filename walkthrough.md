...
이제 [data_collector.py](file:///Applications/stock_dashboard/data_collector.py)는 내부 코드가 아닌 DB의 [watchlist](file:///Applications/stock_dashboard/crud.py#76-86) 테이블을 기준으로 작동합니다.
...
- **작동 방식**:
  1. 검색창에 '카카오' 입력 후 엔터.
  2. 백엔드가 종목코드를 찾아 [watchlist](file:///Applications/stock_dashboard/crud.py#76-86)에 등록.
...
- **검증**: [kis_client.py](file:///Applications/stock_dashboard/kis_client.py) 테스트를 통해 삼성전자(005930)의 현재 시세를 초단위로 정확히 수신함을 확인했습니다.
...
- **데이터 시각화 복구**: 백엔드([processor.py](file:///Applications/stock_dashboard/processor.py))와 프론트엔드([App.jsx](file:///Applications/stock_dashboard/frontend/src/App.jsx)) 간의 데이터 키 불일치(price vs close 등)를 해결하여 모든 그래프가 정상적으로 출력됩니다.
...
- **데이터 투명성 (Database Status)**:
  - 시스템 내 실제 [stock.db](file:///Applications/stock_dashboard/stock.db) 위치 및 실시간 적재 데이터 건수(주가/재무)를 투명하게 공개.
- **기능 최적화**:
...
| # | 파일 | 수정 내용 | 영향 |
| :--- | :--- | :--- | :--- |
| **BF-01** | [main.py](file:///Applications/stock_dashboard/main.py) | `from datetime import datetime` 임포트 추가 | `/api/dashboard/stats` 호출 시 NameError 런타임 오류 해결 |
| **BF-02** | [main.py](file:///Applications/stock_dashboard/main.py) | [get_ready_reports()](file:///Applications/stock_dashboard/main.py#137-167)에서 `stock["code"]` → `stock["stock_code"]` 키 수정 | `/api/reports/ready` 호출 시 KeyError 오류 해결 |
| **BF-03** | [processor.py](file:///Applications/stock_dashboard/processor.py) | [get_sector_performance()](file:///Applications/stock_dashboard/processor.py#67-87)에 `return performance` 추가 | `/api/dashboard/sectors`가 항상 `null` 반환하던 문제 해결 |
| **BF-04** | [kis_client.py](file:///Applications/stock_dashboard/kis_client.py) | 미완성 중복 [get_current_price()](file:///Applications/stock_dashboard/kis_client.py#138-183) 첫 번째 정의 제거 | KIS 현재가 조회 메서드 중복으로 인한 불안정성 제거 |
| **BF-05** | [kis_client.py](file:///Applications/stock_dashboard/kis_client.py) | [get_investor_trends()](file:///Applications/stock_dashboard/kis_client.py#93-137)에 실제 HTTP 요청 코드 완성 | 기관/외국인 수급 데이터 수집 불가 → 정상 수집 가능 |
...
**백엔드 ([main.py](file:///Applications/stock_dashboard/main.py))**
- `DELETE /api/commands/watchlist/{stock_code}` 신규 엔드포인트 추가
  - 와치리스트에서 특정 종목을 제거, 없는 종목 요청 시 404 응답

**프론트엔드 ([App.jsx](file:///Applications/stock_dashboard/frontend/src/App.jsx))**
- 사이드바에 ⭐ **관심종목** 탭 추가
- [WatchlistView](file:///Applications/stock_dashboard/frontend/src/App.jsx#138-255) 컴포넌트 신규 구현:
