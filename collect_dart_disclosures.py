...
def main():
    ...
    def _next_dart():
        _key_idx[0] = (_key_idx[0] + 1) % len(_dart_keys)
        key = _dart_keys[_key_idx[0]]
        log.warning(f"DART API 키 교체 → KEY{_key_idx[0]+1}")
        return OpenDartReader(key)

    dart = OpenDartReader(_dart_keys[0])
    end_dt = datetime.now()
    ...
    for i, code in enumerate(codes, 1):
        ...
        try:
            ...
            if items:
                ...
                empty_streak += 1
                # 연속 5회 이상 빈 결과 → 할당량 초과로 판단, 다음 키로 교체
                if empty_streak >= 5:
                    if key_switches < len(_dart_keys) - 1:
                        dart = _next_dart()
                        key_switches += 1
                        empty_streak = 0
                        log.warning(f"[{i}/{total}] 키 교체 완료 (총 {key_switches}회), 계속 진행")
                    else:
                        log.warning(
                            f"[{i}/{total}] 모든 키({len(_dart_keys)}개) 한도 초과. 중단."
                        )
                        break
