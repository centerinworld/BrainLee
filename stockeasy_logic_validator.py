...
def _calc_se_momentum_candidates(conn: sqlite3.Connection) -> list[dict]:
    ...
    for r in rows:
        ...
        # ── 대형주 보너스 (SE Momentum은 대형 주도주 편향) ──
        size_bonus = 0
        if mktcap >= 5e12:  size_bonus = 5   # 5조 이상
        elif mktcap >= 1e12: size_bonus = 3  # 1조 이상
        elif mktcap >= 3e11: size_bonus = 1  # 3000억 이상
...
def _calc_se_value_candidates(conn: sqlite3.Connection) -> list[dict]:
    ...
    FROM lu
    JOIN px p ON p.stock_code=lu.stock_code AND p.rn=1
    WHERE lu.mktcap >= 100000000000           -- 시총 1000억 이상
    ...
    for r in rows:
        ...
        # ── 시총 보너스 — SE Value는 대형 우량주 편향이 매우 강함 ──
        if mktcap >= 1e14:    score += 10  # 100조 이상 (삼성전자급)
        elif mktcap >= 2e13:  score += 8   # 20조 이상
        elif mktcap >= 1e13:  score += 7   # 10조 이상 (하이브 11.6조)
        elif mktcap >= 5e12:  score += 6   # 5조 이상
        elif mktcap >= 2e12:  score += 5   # 2조 이상 (클래시스 3.4조)
        elif mktcap >= 1e12:  score += 4   # 1조 이상
        elif mktcap >= 3e11:  score += 2   # 3000억 이상
...
def _calc_largecap_leader_candidates(conn: sqlite3.Connection, strategy: str, as_of: Optional[str] = None) -> list[dict]:
    ...
    for r in rows:
        ...
        out.append({
            ...
            "stock_name": r["stock_name"],
            "score": round(score, 2),
            "mktcap_억": round(float(r["mktcap"] or 0) / 1e8, 1),
        })
    return out
...
def _extract_sell_features(
    ...
    flow = conn.execute(
        f"""
        SELECT COALESCE(SUM(frn_net_buy_amt),0)/100.0  AS frn5_억,
               COALESCE(SUM(inst_net_buy_amt),0)/100.0 AS inst5_억
        FROM (
          SELECT frn_net_buy_amt, inst_net_buy_amt
          FROM price_history
          WHERE stock_code=? {flow_filter}
            AND frn_net_buy_amt IS NOT NULL AND inst_net_buy_amt IS NOT NULL
    ...
    flow_sum = frn5 + inst5
    mktcap_억 = mktcap / 1e8 if mktcap > 0 else 0
...
def _get_our_sell_candidates(strategy: str, se_holdings: list[dict], as_of: Optional[str] = None, sell_cfg: Optional[dict] = None) -> list[dict]:
    ...
    try:
        for h in (se_holdings or []):
            ...
            # 대형주(5조+)는 절대 수급금액이 크므로 더 엄격한 임계치 적용
            _flow_tp_cut = -2.0 if feat["mktcap"] >= 5e12 else -0.5
