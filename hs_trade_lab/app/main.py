...
ANALYSIS2_CANONICAL_SECTOR_KEYS = tuple(sector["sector_key"] for sector in DEFAULT_SECTORS)
ANALYSIS2_CANONICAL_SECTOR_PLACEHOLDERS = ",".join("?" for _ in ANALYSIS2_CANONICAL_SECTOR_KEYS)

# ── 경량 인메모리 캐시 (companies 목록 — 일별 갱신 수준이므로 TTL 1시간) ──
_companies_cache: dict = {}
_COMPANIES_CACHE_TTL = 3600  # seconds
...
def _rebuild_analysis2_cache(timeout: int = 180) -> dict:
    """매핑 수정 후 analysis2 캐시(섹터/기업 HS 구성표)를 즉시 재계산.

    사용자가 기업 매핑을 이동/삭제한 뒤 화면에 바로 반영되도록 동기 실행한다
    (기존 /api/customs/daily-refresh 등과 동일한 blocking subprocess 패턴).
    """
    cmd = [
        str(PROJECT_DIR.parent / "venv" / "bin" / "python"),
        str(PROJECT_DIR / "scripts" / "rebuild_analysis2_cache.py"),
    ]
    try:
        proc = subprocess.run(cmd, cwd=PROJECT_DIR, capture_output=True, text=True, timeout=timeout)
        return {"ok": proc.returncode == 0, "output": (proc.stdout or proc.stderr or "")[-800:]}
    except subprocess.TimeoutExpired:
        return {"ok": False, "output": "rebuild timed out"}
...
def _latest_positive_point(monthly: list[dict], value_key: str) -> dict | None:
    for row in sorted(monthly, key=lambda x: x.get("period_ym", ""), reverse=True):
        if row.get("is_provisional") or not _is_month_ym(row.get("period_ym")):
            continue
...
def _metric_snapshot(monthly: list[dict], value_key: str) -> dict:
    clean = [
        row for row in monthly
        if not row.get("is_provisional") and _is_month_ym(row.get("period_ym"))
    ]
    lookup = {row["period_ym"]: float(row.get(value_key) or 0) for row in clean}
    latest = _latest_positive_point(clean, value_key)
    latest_val = float(latest.get(value_key) or 0) if latest else None
    latest_period = latest.get("period_ym") if latest else None

    prev_val = None
    if latest_period:
        for row in sorted(clean, key=lambda x: x["period_ym"], reverse=True):
            if row["period_ym"] >= latest_period:
                continue
    ...
    if latest_period:
        cur_periods = [_shift_month(latest_period, -offset) for offset in range(3)]
        prev_periods = [_shift_month(period, -12) for period in cur_periods]
        pairs = []
        for cur_period, prev_period in zip(cur_periods, prev_periods):
            cur_val = lookup.get(cur_period, 0.0)
            prev_year_val = lookup.get(prev_period, 0.0)
            if cur_val > 0 and prev_year_val > 0:
                pairs.append((cur_period, prev_period, cur_val, prev_year_val))
        if pairs:
            cur_avg = sum(item[2] for item in pairs) / len(pairs)
            prev_avg = sum(item[3] for item in pairs) / len(pairs)
            yoy_payload = {
                "value": _pct(cur_avg, prev_avg),
                "basis": "3m_avg",
                "current_avg": round(cur_avg),
                "previous_avg": round(prev_avg),
                "current_periods": [item[0] for item in pairs],
                "previous_periods": [item[1] for item in pairs],
            }

    return {
        "latest": round(latest_val) if latest_val is not None else None,
        "latest_period": latest_period,
        "mom": _pct(latest_val, prev_val),
        "yoy": yoy_payload["value"],
        "yoy_basis": yoy_payload["basis"],
        "yoy_current_avg": yoy_payload["current_avg"],
        "yoy_previous_avg": yoy_payload["previous_avg"],
        "yoy_current_periods": yoy_payload["current_periods"],
        "yoy_previous_periods": yoy_payload["previous_periods"],
    }
...
def get_sector_presets(db: Session = Depends(get_db)):
    rows = (
        db.query(SectorPreset)
        .filter(SectorPreset.sector_key.in_(ANALYSIS2_CANONICAL_SECTOR_KEYS))
        .order_by(SectorPreset.sort_order)
        .all()
    )
    return [
        SectorPresetOut(
...
@app.patch("/api/mappings/{mapping_id}", response_model=HSCodeMapOut)
def patch_mapping(mapping_id: int, payload: HSCodeMapPatch, db: Session = Depends(get_db)):
    """기업-HS코드 매핑 부분 수정 (다른 HS코드/섹터로 이동, 상태 재분류 등).

    hs_code 또는 stock_code를 바꿔 다른 (hs_code, stock_code) 조합과 충돌하면 거부한다.
    저장 후 analysis2 캐시를 동기적으로 재계산해 화면에 즉시 반영한다.
    """
    ...
    next_hs_code = updates.get("hs_code", row.hs_code)
    next_stock_code = updates.get("stock_code", row.stock_code)
    if (next_hs_code, next_stock_code) != (row.hs_code, row.stock_code):
        conflict = (
            db.query(HSCodeCompanyMap)
            .filter(
                HSCodeCompanyMap.hs_code == next_hs_code,
                HSCodeCompanyMap.stock_code == next_stock_code,
                HSCodeCompanyMap.id != mapping_id,
            )
            .first()
        )
        if conflict:
            raise HTTPException(status_code=409, detail="이동하려는 HS코드-기업 조합이 이미 존재합니다.")
    ...
    rebuild = _rebuild_analysis2_cache()
    result = _to_map_out(row)
    return result
...
def delete_mapping(mapping_id: int, db: Session = Depends(get_db)):
    ...
    db.commit()
    rebuild = _rebuild_analysis2_cache()
    return GenericMessage(message="deleted", detail={"rebuild_ok": rebuild["ok"]})
...
def analysis2_sectors(months: int = 24):
    ...
    rows = conn.execute(
        ...
        WHERE c.period_ym >= date('now', ? || ' months')
          AND c.sector_key IN ({ANALYSIS2_CANONICAL_SECTOR_PLACEHOLDERS})
        ORDER BY sp.sort_order, c.period_ym
        """,
        (f"-{months}", *ANALYSIS2_CANONICAL_SECTOR_KEYS),
    ).fetchall()
    ...
    for sector in sectors.values():
        ...
        export_stats = _metric_snapshot(cm, "export_val")
        import_stats = _metric_snapshot(cm, "import_val")
        export_latest = export_stats["latest"]
        import_latest = import_stats["latest"]
        result.append(
            {
                **sector,
                ...
                "export_latest": export_latest,
                "export_mom": export_stats["mom"],
                "export_yoy": export_stats["yoy"],
                "export_yoy_basis": export_stats["yoy_basis"],
                "export_yoy_current_avg": export_stats["yoy_current_avg"],
                "export_yoy_previous_avg": export_stats["yoy_previous_avg"],
                "export_yoy_current_periods": export_stats["yoy_current_periods"],
                "export_yoy_previous_periods": export_stats["yoy_previous_periods"],
                "import_latest": import_latest,
                "import_mom": import_stats["mom"],
                "import_yoy": import_stats["yoy"],
                "import_yoy_basis": import_stats["yoy_basis"],
                "import_yoy_current_avg": import_stats["yoy_current_avg"],
                "import_yoy_previous_avg": import_stats["yoy_previous_avg"],
                "import_yoy_current_periods": import_stats["yoy_current_periods"],
                "import_yoy_previous_periods": import_stats["yoy_previous_periods"],
...
def analysis2_all_companies(months: int = 3):
    ...
    캐시: 결과를 1시간 인메모리 캐싱 (일별 갱신 빈도에 충분).
    인덱스: ix_analysis2_company_period_export (period_ym, ...) → SEARCH 적용.
    """
    cache_key = f"companies_{months}"
    cached = _companies_cache.get(cache_key)
    if cached and (time.time() - cached["at"]) < _COMPANIES_CACHE_TTL:
        return cached["data"]
    ...
    rows = conn.execute(
        ...
        WHERE period_ym >= date('now', ? || ' months')
          AND sector_key IN ({ANALYSIS2_CANONICAL_SECTOR_PLACEHOLDERS})
    ...
    _companies_cache[cache_key] = {"data": result, "at": time.time()}
    return result


@app.get("/api/analysis2/company/{stock_code}/by-product")
def analysis2_company_by_product(stock_code: str, months: int = 24, period_ym: str | None = None):
    ...
    all_periods = [r["period_ym"] for r in periods_rows]
    if period_ym and period_ym in all_periods:
        end_period = period_ym
    else:
        end_period = all_periods[0] if all_periods else None

    # 선택한 기준월을 포함한 최근 months 구간만 사용
    if end_period:
        start_period_row = conn.execute(
            """
            SELECT strftime('%Y-%m', date(? || '-01', ? || ' months')) AS start_period
            """,
            (end_period, f"-{max(0, months - 1)}"),
        ).fetchone()
        start_period = start_period_row["start_period"] if start_period_row else None
    else:
        start_period = None
    ...
    if start_period and end_period:
        hs_rows = conn.execute(
            ...
            """,
            (stock_code, start_period, end_period),
        ).fetchall()
    else:
        hs_rows = []
    # 기업 기본정보
    ...
    if not hs_rows:
        ...
        if top_sectors:
            ...
            hs_rows = conn.execute(
                ...
                ORDER BY period_ym DESC, export_value DESC
                """,
                (*top_sectors, start_period or "0000-01", end_period or "9999-12"),
            ).fetchall()
    ...
    items = sorted(
        hs_summary.values(),
        key=lambda x: -((x["export_3m"] or 0) if x["flow_type"] != "import" else (x["import_3m"] or 0)),
    )[:30]
    for item in items:
    ...
    export_items = [item for item in items if item["flow_type"] != "import"]
    import_items = [item for item in items if item["flow_type"] == "import"]
    if is_sector_fallback and not import_items:
        import_items = [
            {
                **item,
                "flow_type": "import",
            ...
            for item in items
            if (item.get("import_3m") or 0) > 0
        ]

    # allocation_basis: 데이터 할당 방식을 명시 (프론트/텔레그램 렌더러 신뢰성 보장)
    if is_sector_fallback:
        allocation_basis = "equal_split"   # 섹터 수준 데이터, 기업별 점유율 미확정
    elif share_map:
        allocation_basis = "market_share"  # hs_code_company_map 기반 점유율 적용
    else:
        allocation_basis = "unknown"       # 데이터 있으나 점유율 정보 없음
...
def analysis2_company_trend(stock_code: str, months: int = 24, sector_key: str | None = None):
    ...
    if provisional and provisional.get("period_ym", "") > confirmed_latest_ym and monthly:
        ...
        sector_rows_3m = conn.execute(
            ...
            """,
            (prov_sector, monthly[-3]["period_ym"] if len(monthly) >= 3 else monthly[-1]["period_ym"]),
        ).fetchone() if prov_sector else None
...
def analysis2_company_hs_breakdown(stock_code: str, sector_key: str, period_ym: str | None = None):
    ...
    if not chosen_period:
        ...
        for r in fallback_rows:
            exp_v = round(r["export_value"] or 0)
            imp_v = round(r["import_value"] or 0)
            if exp_v > 0:
                items.append(
                    {
                        "hs_code": r["hs_code"], "hs_name": r["hs_name"], "sector_name": "섹터 추정",
                        "flow_type": "export", "mapping_status": r["mapping_status"],
                        "export_val": exp_v, "export_kg": round(r["export_weight"] or 0),
                        "import_val": imp_v, "import_kg": round(r["import_weight"] or 0),
            ...
            if imp_v > 0:
                items.append(
                    {
                        "hs_code": r["hs_code"], "hs_name": r["hs_name"], "sector_name": "섹터 추정",
                        "flow_type": "import", "mapping_status": r["mapping_status"],
                        "export_val": exp_v, "export_kg": round(r["export_weight"] or 0),
                        "import_val": imp_v, "import_kg": round(r["import_weight"] or 0),
                        "export_share": None,
        ...
        return {
            ...
            "is_sector_fallback": True,
            "allocation_basis": "equal_split",  # 섹터 대체 데이터 — 기업별 점유율 미확정
    ...
    return {
        ...
        "period_ym": chosen_period,
        "periods": [row["period_ym"] for row in periods],
        "allocation_basis": "market_share",  # 기업별 HS 캐시 사용 — 개별 데이터 직접 반영
        "is_sector_fallback": False,
        "items": items,
...
def analysis2_stock_export_summary(stock_code: str, months: int = 12):
    ...
    export_stats = _metric_snapshot(monthly, "export_val")
    export_latest = export_stats["latest"]
...
def analysis2_export_health(months: int = 6):
    ...
    # ── 섹터 건강 점수: 최신 단월 대신 최근 유효 3개월 평균 YoY ─────
    sector_monthly_all = conn.execute("""
        SELECT sector_key, sector_label, period_ym, export_value, import_value
        FROM analysis2_sector_monthly_cache
        WHERE period_ym LIKE '____-__'
        ORDER BY sector_key, period_ym
    """).fetchall()
    sector_groups: dict[str, dict] = {}
    for r in sector_monthly_all:
        group = sector_groups.setdefault(
            r["sector_key"],
            {"sector_key": r["sector_key"], "sector_label": r["sector_label"], "monthly": []},
        )
    ...
    for group in sector_groups.values():
        export_stats = _metric_snapshot(group["monthly"], "export_val")
        import_stats = _metric_snapshot(group["monthly"], "import_val")
        sector_rows.append({
            "sector_key": group["sector_key"],
            "sector_label": group["sector_label"],
            "period_ym": export_stats["latest_period"] or import_stats["latest_period"],
            "export_value": export_stats["latest"],
            "import_value": import_stats["latest"],
            "export_mom": export_stats["mom"],
            "export_yoy": export_stats["yoy"],
            "import_mom": import_stats["mom"],
            "import_yoy": import_stats["yoy"],
    ...
    sector_rows.sort(key=lambda r: r["export_value"] or 0, reverse=True)
    ...
    # ── 기업 건강 점수 ──────────────────────────────────────────
    company_monthly_all = conn.execute("""
        SELECT stock_code, MAX(stock_name) AS stock_name, MAX(sector_label) AS sector_label,
               period_ym, SUM(export_value) AS export_val, SUM(import_value) AS import_val
        FROM analysis2_company_monthly_cache
        WHERE period_ym LIKE '____-__'
        GROUP BY stock_code, period_ym
        ORDER BY stock_code, period_ym
    """).fetchall()
    company_groups: dict[str, dict] = {}
    for r in company_monthly_all:
        ...
        group["monthly"].append({
            "period_ym": r["period_ym"],
            "export_val": round(r["export_val"] or 0),
            "import_val": round(r["import_val"] or 0),
        })
    company_rows = []
    for group in company_groups.values():
        export_stats = _metric_snapshot(group["monthly"], "export_val")
        import_stats = _metric_snapshot(group["monthly"], "import_val")
        if not export_stats["latest"] or export_stats["latest"] <= 1_000_000:
            continue
        company_rows.append({
            ...
            "sector_label": group["sector_label"],
            "period_ym": export_stats["latest_period"] or import_stats["latest_period"],
            "export_val": export_stats["latest"],
            "import_val": import_stats["latest"],
            "export_mom": export_stats["mom"],
            "export_yoy": export_stats["yoy"],
            "import_mom": import_stats["mom"],
            "import_yoy": import_stats["yoy"],
    ...
    company_rows.sort(key=lambda r: r["export_yoy"] if r["export_yoy"] is not None else -999999, reverse=True)
    company_rows = company_rows[:100]
        ...
        "sectors": [
            {
                ...
                "export_yoy_basis": r.get("export_yoy_basis"),
                "import_mom": r["import_mom"], "import_yoy": r["import_yoy"],
                "import_yoy_basis": r.get("import_yoy_basis"),
...
def analysis2_hs_companies(hs_code: str, sector_key: str | None = None):
    """특정 HS 코드를 공유하는 기업 목록 + 시장비율.

    프론트의 "HS코드 클릭 → 기업 목록 → 이동/삭제" 관리 도구에서 사용.
    각 기업 항목에 매핑 id/note/confidence를 포함해 그대로 수정·삭제 API에 넘길 수 있게 한다.
    sector_key가 주어지면 그 섹터의 hs_sector_map 행(제외 버튼용 id 포함)도 함께 반환한다.
    """
    conn = _hs_db()
    rows = conn.execute(
        """
        SELECT hcm.id, hcm.stock_code, hcm.stock_name, hcm.sector_name, hcm.mapping_status,
               hcm.flow_type, hcm.confidence, hcm.note, hcm.market_share_pct,
        ...
        WHERE hcm.hs_code = ?
        ORDER BY hcm.market_share_pct DESC NULLS LAST,
                 CASE hcm.mapping_status WHEN 'exact' THEN 1 WHEN 'composite' THEN 2 ELSE 3 END,
                 hcm.stock_name
        """,
        (hs_code,),
    ).fetchall()
    ...
    all_sector_rows = conn.execute(
        """
        SELECT sm.id, sm.sector_key, sp.label AS sector_label, sm.mapping_status, sm.display_name
        FROM hs_sector_map sm
        LEFT JOIN sector_preset sp ON sp.sector_key = sm.sector_key
        WHERE sm.hs_code = ?
        ORDER BY sm.sector_key
        """,
        (hs_code,),
    ).fetchall()
    current_sector_map = None
    if sector_key:
        current_sector_map = next((r for r in all_sector_rows if r["sector_key"] == sector_key), None)
    conn.close()
    return {
        "hs_info": {
            "hs_code": hs_code,
        ...
        "sector_maps": [
            {
                "id": r["id"], "sector_key": r["sector_key"], "sector_label": r["sector_label"],
                "mapping_status": r["mapping_status"], "display_name": r["display_name"],
            }
            for r in all_sector_rows
        ],
        "current_sector_map": (
            {
                "id": current_sector_map["id"], "sector_key": current_sector_map["sector_key"],
                "mapping_status": current_sector_map["mapping_status"],
            }
            if current_sector_map else None
        ),
        "companies": [
            {
                "id": r["id"],
