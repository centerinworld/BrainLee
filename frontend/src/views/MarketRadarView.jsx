...
const MarketRadarView = React.memo(() => {
  ...
  React.useEffect(() => {
    let alive = true;
    fetch(API('/api/sector-rotation/leadership?months=36&top_n=3'))
      .then(r => r.ok ? r.json() : null)
      .then(d => { if (alive && d) setLeadership(d); })
      .catch(() => {});
    return () => { alive = false; };
  }, []);
  ...
  const stageStyle = (stage) => ({
    ENTRY_NOW:   { bg:'rgba(34,197,94,0.16)',  border:'#22c55e', color:'#4ade80' },
    EARLY_WATCH: { bg:'rgba(251,191,36,0.16)', border:'#fbbf24', color:'#fbbf24' },
    HOLD_LEADER: { bg:'rgba(59,130,246,0.14)', border:'#3b82f6', color:'#93c5fd' },
    WAIT:        { bg:'rgba(100,116,139,0.10)',border:'#475569', color:'#94a3b8' },
    AVOID:       { bg:'rgba(239,68,68,0.12)',  border:'#ef4444', color:'#f87171' },
  }[stage] || { bg:'rgba(100,116,139,0.10)', border:'#475569', color:'#94a3b8' });
  ...
  return (
    <div style={{padding:'0 0.5rem'}}>
      ...
      {/* ── 주도섹터 진입 신호 ───────────────────────────────────── */}
      {leadership?.sectors?.length > 0 && (
        <div className="glass-panel" style={{ margin:'0.65rem 0 0.8rem', padding:'0', overflow:'hidden' }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:'0.8rem', padding:'0.75rem 0.9rem', borderBottom:'1px solid var(--glass-border)', flexWrap:'wrap' }}>
            <div style={{ display:'flex', alignItems:'baseline', gap:'0.65rem', flexWrap:'wrap' }}>
              <strong style={{ color:'var(--text-primary)', fontSize:'0.92rem' }}>🚦 주도섹터 진입 신호</strong>
              <span style={{ color:'var(--text-secondary)', fontSize:'0.74rem' }}>
                진입 {leadership.summary?.entry_now || 0} · 초기 관찰 {leadership.summary?.watch || 0} · 기준일 {leadership.as_of}
              </span>
            </div>
          </div>
          <div style={{ overflowX:'auto' }}>
            <table style={{ width:'100%', minWidth:920, borderCollapse:'collapse', fontSize:'0.75rem' }}>
              <thead>
                <tr>
                  {['단계','섹터','점수/국면','핵심 근거','수급 3M','RS','주도주','강신호 월'].map(h => (
                    <th key={h} style={{ padding:'0.45rem 0.55rem', color:'var(--text-secondary)', fontWeight:700, textAlign:'left', borderBottom:'1px solid var(--glass-border)', background:'rgba(0,0,0,0.22)', whiteSpace:'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {leadership.sectors.map(s => {
                  ...
                  const d = s.detail || {};
                  const leaders = (s.leaders || []).slice(0, 3);
                  return (
                    <tr key={s.sector} style={{ borderBottom:'1px solid rgba(255,255,255,0.05)', background:s.stage === 'ENTRY_NOW' ? 'rgba(34,197,94,0.04)' : 'transparent' }}>
                      <td style={{ padding:'0.52rem 0.55rem', whiteSpace:'nowrap' }}>
                        <span style={{ background:st.bg, border:`1px solid ${st.border}`, color:st.color, borderRadius:6, padding:'0.12rem 0.45rem', fontWeight:800 }}>{s.stage_label}</span>
                      </td>
                      <td style={{ padding:'0.52rem 0.55rem', color:'var(--text-primary)', fontWeight:800, whiteSpace:'nowrap' }}>{s.label}</td>
                      <td style={{ padding:'0.52rem 0.55rem', whiteSpace:'nowrap' }}>
                        <span style={{ color:st.color, fontWeight:900 }}>{s.score}점</span>
                        <span style={{ color:'var(--text-secondary)', marginLeft:6 }}>{s.phase}</span>
                      </td>
                      <td style={{ padding:'0.52rem 0.55rem', color:'#cbd5e1', minWidth:170 }}>{(s.entry_reasons || []).slice(0, 3).join(' · ')}</td>
                      <td style={{ padding:'0.52rem 0.55rem', whiteSpace:'nowrap' }}>
                        <span style={{ color:(d.frn_3m_억 || 0) > 0 ? '#4ade80' : '#f87171' }}>외인 {fmtEok(d.frn_3m_억)}</span>
                        <span style={{ color:(d.inst_3m_억 || 0) > 0 ? '#60a5fa' : '#f87171', marginLeft:8 }}>기관 {fmtEok(d.inst_3m_억)}</span>
                      </td>
                      <td style={{ padding:'0.52rem 0.55rem', whiteSpace:'nowrap' }}>
                        <span style={{ color:(s.rs4w || 0) > 0 ? '#4ade80' : '#f87171' }}>4W {fmtPct(s.rs4w)}</span>
                        <span style={{ color:(s.rs12w || 0) > 0 ? '#4ade80' : '#f87171', marginLeft:8 }}>12W {fmtPct(s.rs12w)}</span>
                      </td>
                      <td style={{ padding:'0.52rem 0.55rem', minWidth:220 }}>
                        {leaders.map((p, i) => (
                          <span key={p.code} style={{ display:'inline-block', marginRight:8, color:i === 0 ? '#fbbf24' : '#e2e8f0', fontWeight:i === 0 ? 800 : 600 }}>
                            {i + 1}. {p.name} <span style={{ color:'var(--text-secondary)' }}>{p.surge_score}</span>
                          </span>
                        ))}
                      </td>
                      <td style={{ padding:'0.52rem 0.55rem', color:'#cbd5e1', whiteSpace:'nowrap' }}>
                        {s.peak_signal ? `${s.peak_signal.month} · ${s.peak_signal.score}점` : '-'}
                      </td>
                    </tr>
                  );
                })}
