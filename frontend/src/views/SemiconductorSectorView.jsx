...
const StockPerformancePanel = ({
  ...
  React.useEffect(() => {
    if (!pickedCode) { setFinDetail(null); return; }
    let cancelled = false;
    fetch(`/api/market-radar/semiconductor/financial-detail?stock_code=${pickedCode}`)
      .then(r => r.ok ? r.json() : null)
      .then(d => { if (!cancelled) setFinDetail(d?.ok ? d : null); })
      .catch(() => { if (!cancelled) setFinDetail(null); });
    return () => { cancelled = true; };
  }, [pickedCode]);

  const cfsCapex = finDetail?.cashflow?.cfs?.capex ?? finDetail?.cashflow?.ofs?.capex ?? null;
  const cfsDepr = finDetail?.cashflow?.cfs?.depreciation ?? finDetail?.cashflow?.ofs?.depreciation ?? null;
  const ttmRev = finDetail?.ttm?.revenue ?? null;
  const ttmNi = finDetail?.ttm?.net_income ?? null;
  const epsVal = finDetail?.eps ?? null;
  const perTtm = finDetail?.per_ttm ?? null;
  const matAmt = finDetail?.material_purchase?.amount_uk ?? null;
  const matYear = finDetail?.material_purchase?.year ?? null;
  const backlogAmt = finDetail?.order_backlog?.amount_uk ?? null;
  const backlogPeriod = finDetail?.order_backlog?.period ?? null;
          ...
          <div style={{height:'260px'}}>
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={annualSeries}>
                ...
                <YAxis yAxisId="r" orientation="right" domain={['auto', 'auto']} tick={{fill:'#94a3b8', fontSize:11}} />
                <Tooltip />
                <Legend />
                <ReferenceLine y={0} yAxisId="r" stroke="rgba(148,163,184,0.4)" strokeDasharray="4 2" />
                <Bar yAxisId="l" dataKey="revenue" name="매출(억)" fill="#22d3ee" radius={[4,4,0,0]} />
                <Line yAxisId="r" type="monotone" dataKey="profit" name="이익률(%)" stroke="#f97316" strokeWidth={2} dot={{r:3}} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div style={{height:'260px'}}>
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={quarterSeries}>
                ...
                <Line yAxisId="r" type="monotone" dataKey="profit" name="이익률(%)" stroke="#fb7185" strokeWidth={2} dot={{r:3}} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="semi-table-wrap">
        <table className="semi-excel-table" style={{minWidth:'860px'}}>
          ...
          <tbody>
            ...
            <tr>
              <td style={{...cellBase, background:'#e0f2fe', fontWeight:800, color:'#0c4a6e'}} colSpan={4}>
                ── 추가 재무 지표 ──
              </td>
            </tr>
            <tr>
              <td style={{...cellBase, background:'#eef2ff', fontWeight:800, color:'#1e293b'}}>TTM 매출 / 이익(억)</td>
              <td style={{...cellBase, background:'#fff', textAlign:'right', fontWeight:700}}>
                {(ttmRev != null || ttmNi != null) ? `${ttmRev != null ? fmtNum(ttmRev) : '-'} / ${ttmNi != null ? fmtNum(ttmNi) : '-'}` : '-'}
              </td>
              <td style={{...cellBase, background:'#eef2ff', fontWeight:800, color:'#1e293b'}}>EPS / TTM PER</td>
              <td style={{...cellBase, background:'#fff', textAlign:'right', fontWeight:700}}>
                {epsVal != null ? `${fmtNum(Math.round(epsVal))}원` : '-'} / {perTtm != null ? `${perTtm}배` : '-'}
              </td>
            </tr>
            <tr>
              <td style={{...cellBase, background:'#eef2ff', fontWeight:800, color:'#1e293b'}}>Forward EPS / Forward PER</td>
              <td style={{...cellBase, background:'#fff', textAlign:'right', fontWeight:700, color:'#94a3b8'}}>-</td>
              <td style={{...cellBase, background:'#eef2ff', fontWeight:800, color:'#1e293b'}}>CapEx 투자규모(억)</td>
              <td style={{...cellBase, background:'#fff', textAlign:'right', fontWeight:700}}>
                {cfsCapex != null ? fmtNum(cfsCapex) : '-'}
              </td>
            </tr>
            <tr>
              <td style={{...cellBase, background:'#eef2ff', fontWeight:800, color:'#1e293b'}}>감가상각비(억)</td>
              <td style={{...cellBase, background:'#fff', textAlign:'right', fontWeight:700}}>
                {cfsDepr != null ? fmtNum(cfsDepr) : '-'}
              </td>
              <td style={{...cellBase, background:'#eef2ff', fontWeight:800, color:'#1e293b'}}>매입재료비(억){matYear ? ` ${matYear}` : ''}</td>
              <td style={{...cellBase, background:'#fff', textAlign:'right', fontWeight:700}}>
                {matAmt != null ? fmtNum(matAmt) : '-'}
              </td>
            </tr>
            <tr>
              <td style={{...cellBase, background:'#eef2ff', fontWeight:800, color:'#1e293b'}}>수주잔고(억){backlogPeriod ? ` ${backlogPeriod}` : ''}</td>
              <td style={{...cellBase, background:'#fff', textAlign:'right', fontWeight:700}}>
                {backlogAmt != null ? fmtNum(backlogAmt) : '-'}
              </td>
              <td style={{...cellBase, background:'#eef2ff', fontWeight:800, color:'#1e293b'}}></td>
              <td style={{...cellBase, background:'#fff', textAlign:'right'}}></td>
            </tr>
          </tbody>
        </table>
      </div>
