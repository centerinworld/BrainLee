...
const SemiconductorView = React.memo(() => {
  ...
  // financial-detail + financial-history 병렬 패치
  React.useEffect(() => {
    if (!pickedCode) { setFinDetail(null); setHistData(null); return; }
    const sc = encodeURIComponent(pickedCode);
    Promise.allSettled([
      fetch(`/api/market-radar/semiconductor/financial-detail?stock_code=${sc}`).then(r => r.ok ? r.json() : null),
      fetch(`/api/market-radar/semiconductor/financial-history?stock_code=${sc}`).then(r => r.ok ? r.json() : null),
    ]).then(([det, hist]) => {
      const d = det.value;
      const h = hist.value;
      setFinDetail(d?.ok ? d : null);
      setHistData(h?.ok ? h : null);
    });
  }, [pickedCode]);

  // 재무이력표: 데이터/탭 변경 시 스크롤을 오른쪽 끝(최신)으로
  React.useEffect(() => {
    if (histTableRef.current) {
      histTableRef.current.scrollLeft = histTableRef.current.scrollWidth;
    }
  }, [histData, histTab]);
  ...
  const annualSeries = React.useMemo(() => {
    ...
    return base.map(r => ({
      ...r,
      margin_pct: (r.revenue != null && r.revenue !== 0 && r.profit != null) ? Number((r.profit / r.revenue * 100).toFixed(1)) : null,
    }));
  ...
  // 연간/분기 차트 도메인 — 두 축의 0을 같은 수직 위치로 정렬
  const _calcChartDomains = (series) => {
    if (!series.length) return null;
    const maxRev = Math.max(...series.map(r => r.revenue || 0), 1);
    const margins = series.map(r => r.margin_pct).filter(v => v != null && isFinite(v));
    if (!margins.length) return null;
    const opmMin = Math.min(...margins) - 5;
    const opmMax = Math.max(Math.max(...margins) + 5, 5);
    const leftMin = opmMin < 0 && opmMax > 0
      ? -(maxRev * Math.abs(opmMin) / opmMax)
      : 0;
    return { leftMin: Math.floor(leftMin), leftMax: Math.ceil(maxRev * 1.1), opmMin: Math.floor(opmMin), opmMax: Math.ceil(opmMax) };
  };
  const annualChartDomains  = React.useMemo(() => _calcChartDomains(annualSeries),      [annualSeries]);
  const quarterChartDomains = React.useMemo(() => _calcChartDomains(quarterSeriesSimple), [quarterSeriesSimple]);
                ...
                <table style={{width:'100%', borderCollapse:'collapse', fontSize:'0.8rem', minWidth:'760px'}}>
                  ...
                  <tbody>
                    {[
                      ...
                      ['TTM 매출(억)', finDetail?.ttm?.revenue != null ? fmtEok(finDetail.ttm.revenue) : '-', 'TTM 순이익(억)', finDetail?.ttm?.net_income != null ? fmtEok(finDetail.ttm.net_income) : '-'],
                      ['EPS(원)', finDetail?.eps != null ? Math.round(finDetail.eps).toLocaleString('ko-KR') : '-', 'TTM PER', finDetail?.per_ttm != null ? `${finDetail.per_ttm}배` : '-'],
                      ['CapEx(억)', (() => { const v = finDetail?.cashflow?.cfs?.capex ?? finDetail?.cashflow?.ofs?.capex; return v != null ? fmtEok(v) : '-'; })(), '감가상각비(억)', (() => { const v = finDetail?.cashflow?.cfs?.depreciation ?? finDetail?.cashflow?.ofs?.depreciation; return v != null ? fmtEok(v) : '-'; })()],
              ...
              {histData && (() => {
                ...
                return (
                  <div style={{marginTop:'1.4rem'}}>
                    ...
                    <div ref={histTableRef} style={{overflowX:'auto'}}>
                      <table style={{width:'100%', borderCollapse:'collapse', fontSize:'0.77rem'}}>
                        ...
                        <tbody>
                          {rowDefs.map((row) => (
                            <tr key={row.label}>
                              <td style={lbStyle}>{row.label}</td>
                              {cols.map(c => {
                                const val = row.fn(c.data);
                                const color = row.clr(c.data);
                                return (
                                  <td key={c.key} style={{
                                    padding:'6px 7px',
