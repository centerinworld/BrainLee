...
const MarketIndicatorsView = React.memo(({ onChangeStock, onChangeTab }) => {
  ...
  const loadShortForeign = React.useCallback(() => {
    fetch(API('/api/market-indicators/short-foreign?days=120'))
      ...
      .catch(() => setShortForeign(null));
  }, []);

  const loadShortMonthly = React.useCallback(() => {
    fetch(API('/api/market-indicators/short-monthly?months=24'))
      .then(r => r.ok ? r.json() : null)
      .then(setShortMonthly)
      .catch(() => setShortMonthly(null));
  }, []);
  ...
  React.useEffect(() => {
    ...
    loadShortForeign();
    loadShortMonthly();
  }, [loadShortForeign, loadShortMonthly]);

  // 내외국인대차 탭 활성화 시 재조회
  React.useEffect(() => {
    if (miTab !== 'short_forg') return;
    loadShortForeign();
    loadShortMonthly();
  }, [miTab, loadShortForeign, loadShortMonthly]);

  // 탭 활성화 중 주기 갱신 + 창 복귀 시 재조회
  React.useEffect(() => {
    if (miTab !== 'short_forg') return;
      ...
      {miTab === 'cash' && (
        <div>
          <div style={{display:'flex',gap:'0.5rem',marginBottom:'1rem',alignItems:'center',flexWrap:'wrap'}}>
            ...
            {[[60,'60일'],[180,'6개월'],[365,'1년'],[1095,'3년']].map(([d,l])=> (
            ...
            <span style={{fontSize:'0.72rem',color:'var(--text-secondary)'}}>
              출처: {(marketCash?.source || '').startsWith('ecos') ? '한국은행 ECOS' : '네이버 증시자금동향'} · 좌축(고객예탁금)=조 · 우축(신용융자)=억
            </span>
          </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
          ...
          <div className="glass-panel" style={{padding:'1rem'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:'0.8rem',marginBottom:'0.8rem',flexWrap:'wrap'}}>
              <h3 style={{margin:0,fontSize:'0.9rem',fontWeight:700}}>내외국인 대차잔고 비교 (최근 120일)</h3>
              <div style={{display:'flex',alignItems:'center',gap:'0.6rem'}}>
                <span style={{fontSize:'0.72rem',color:'var(--text-secondary)'}}>
                  최신기준일: {shortForeign?.latest_balance_date || '-'} / 거래량 {shortForeign?.latest_trade_date || '-'}
                </span>
                <button onClick={loadShortForeign} style={{
                  padding:'0.2rem 0.6rem',borderRadius:'6px',border:'1px solid var(--glass-border)',
                  background:'var(--glass-bg)',color:'var(--text-primary)',fontSize:'0.75rem',cursor:'pointer'
                }}>새로고침</button>
              </div>
            </div>
