...
async def refresh_contracts(payload: dict = {}):
    """즉시 수집 (당일 공시 재스캔). AI 요약/텔레그램 발송 없이 백그라운드 실행."""
    ...
    def _run():
        ...
        try:
            ...
            results = collect_dart_contracts(days=days, min_signal=99, skip_ai=True)
            logger.info(f"[DART수주/refresh] {len(results)}건 처리 (AI/텔레그램 OFF)")
