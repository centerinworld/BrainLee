...
# ══════════════════════════════════════════════════════════════════
# ② Logic-#2 상수 — 수급 주도 모멘텀 (Supply-Led Momentum)
# ══════════════════════════════════════════════════════════════════
# 설계 원칙:
#   기관/외국인 누적 매수 = 1차 신호 (가중치 3배)
#   다중 타임프레임 추세 일봉+주봉(10년) = 2차 확인 (가중치 2배)
#   실적 모멘텀 연속 분기 YoY 성장 = 3차 품질 (가중치 2배)
#   상대강도 RS 1M/3M/6M = 4차 확인 (가중치 1배)
#   점수 = S×3 + T×2 + Q×2 + R×1  (최대 18+12+8+4 = 42점)

# ── 유동성 필터 ────────────────────────────────────────────────────
V2_MKTCAP_MIN         = 100_000_000_000  # 1,000억 (잡주 완전 제거)
V2_TVOL_MIN_KRW       = 10_000_000_000   # 100억 (5일 평균 거래대금)
V2_MAX_FROM_HIGH52    = -35              # 52주 고점 대비 최대 하락폭 (붕괴주 제거)

# ── Track S: 수급 (0~6점) ──────────────────────────────────────────
V2_SUPPLY_CONSEC_BOTH = 3                # 기관+외국인 동반 연속 매수일 (최고등급)
V2_SUPPLY_CONSEC_ONE  = 5                # 기관 or 외국인 단독 연속 매수일 (최고등급)
V2_SUPPLY_AMT_STRONG  = 500              # 10일 누적 순매수 강력 기준 (억원)
V2_SUPPLY_AMT_MID     = 100              # 10일 누적 순매수 중간 기준 (억원)

# ── Track T: 추세 (0~6점) ─────────────────────────────────────────
V2_TREND_DAILY_FULL   = 4                # 일봉 완전 정배열 점수 (5>20>60>120>200)
V2_TREND_WEEKLY_BONUS = 2                # 주봉 40주선+80주선 정배열 보너스
V2_TREND_HIGH52_MIN   = -10              # 52주 고점 10% 이내 = 신고가 근접

# ── Track Q: 실적 품질 (0~4점) ────────────────────────────────────
V2_QUALITY_CONSEC_3Q  = 4                # 연속 3분기 YoY 영업이익 성장 (최고등급)
V2_QUALITY_CONSEC_2Q  = 3                # 연속 2분기 YoY 성장
V2_QUALITY_CONSEC_1Q  = 2                # 최신 분기 YoY 성장
V2_QUALITY_POSITIVE   = 1                # 최신 분기 흑자 (성장 미확인)

# ── Track R: 상대강도 (0~4점) ─────────────────────────────────────
V2_RS_PERIODS         = [21, 63, 126]    # 1M(21일), 3M(63일), 6M(126일)
V2_RS_ALL_THREE       = 4                # 1M+3M+6M 모두 KOSPI 상회
V2_RS_TWO             = 3                # 3M+6M 상회
V2_RS_ONE_MID         = 2                # 3M 상회
V2_RS_ONE_SHORT       = 1                # 1M만 상회

# ── 시그널 임계값 ─────────────────────────────────────────────────
V2_SCORE_STRONG       = 28               # 강력추천: S≥4 AND T≥4 AND score≥28
V2_SCORE_BUY          = 20               # 추천:     S≥3 AND T≥3 AND score≥20
V2_SCORE_WATCH        = 13               # 관심:     S≥2 AND score≥13
V2_SUPPLY_MIN_SIGNAL  = 2                # 시그널 발생 최소 수급 점수
V2_TREND_MIN_SIGNAL   = 2                # 시그널 발생 최소 추세 점수
...
def _high_profit_desc():
    """고수익 집중형 설명 — profit-objective 연구 기반."""
    return [
        {
            "title": "연구 목적함수",
            "desc": (
                "승률이 아니라 12개월 평균수익, 중앙값, profit factor, -30% 손실률을 함께 최적화. "
                "2021~2025 월말 신호 연구에서 평균 +136.27%, 중앙값 +88.27% 조합을 실시간화."
            ),
        },
        {
            "title": "핵심 성장 섹터",
            "desc": "IT, 의료, 경기소비재, 산업재만 포함. 큰 수익 후보가 반복적으로 몰린 섹터군.",
        },
        {
            "title": "유동성 게이트",
            "desc": "최근 20거래일 평균 거래대금 20억원 이상. 실제 자금 진입과 슬리피지 방어를 위한 최소 조건.",
        },
        {
            "title": "52주 고점 근접",
            "desc": "현재가가 52주 고점의 80% 이상. 낙폭주가 아니라 시장이 이미 재평가 중인 강한 종목을 고른다.",
        },
        {
            "title": "내부자 매수",
            "desc": "최근 180일 임원/주요주주 보유 증가. 정보 우위가 있는 내부자의 확신 신호로 가중.",
        },
        {
            "title": "수주잔고/신규수주",
            "desc": "최신 18개월 수주잔고·신규수주 또는 최근 1년 긍정적 공급계약. 향후 매출 가시성 필터.",
        },
        {
            "title": "리스크 배제",
            "desc": "ETF/ETN, 부채비율 500% 초과, 자본잠식은 제외. 위험 공시/거래정지 데이터 연동 시 추가 배제 권장.",
        },
    ]
...
SCREENER_REGISTRY = {
    ...
    "high_profit": {
        "id":            "high_profit",
        "tab_label":     "🏆 고수익 집중",
        "title":         "고수익 집중형 — 내부자 매수 × 수주잔고 × 신고가 근접",
        "subtitle":      "핵심 성장 섹터 × 20일 평균 거래대금 20억↑ × 52주 고점 80%↑ × 내부자 매수 × 수주/신규수주",
        "accent_color":  "#22c55e",
        "badge_text":    "2021~2025 연구: 평균 12개월 수익 +136.27%, 중앙값 +88.27%, 3배 이상 40.43%",
        "footnote":      "희소한 고수익 집중형 후보. 개별 공시·수주 품질과 리스크 공시는 별도 확인 필수.",
        "endpoint":      "/api/signals/high-profit-candidates",
        "items":         _high_profit_desc(),
    },
}
