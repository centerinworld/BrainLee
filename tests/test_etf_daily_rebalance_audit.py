...
from daily_rebalance_audit import audit_day  # noqa: E402
...
class DailyRebalanceAuditTest(unittest.TestCase):
    def test_compares_healthy_etf_while_another_is_unavailable(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for day,ticker,status,count in [
                ("20260828","100001","success",1),
                ("20260901","100001","success",2),
                ("20260901","100002","empty",0),
            ]:
                conn.execute(
                    """
                    ...
                    """,
                    (day,ticker,ticker,"KR7","success" if status == "success" else status,count,"now"),
                )
            conn.executemany(
                """
                INSERT INTO etf_pdf_full_component(
                    base_date,etf_ticker,component_order,component_code,
                    component_name,shares_per_cu,weight
                ) VALUES(?,?,?,?,?,?,?)
                """,
                [
                    ("20260828","100001",1,"005930","Samsung",1,10),
                    ("20260901","100001",1,"005930","Samsung",2,20),
                    ("20260901","100001",2,"000660","SK",1,5),
                ],
            )
            conn.commit()
