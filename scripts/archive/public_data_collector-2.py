...
def collect_short_sell(conn, bas_dt: str) -> int:
    ...
    # GetStocLendBorrInfoService/getStItemLendAndBorrStatu (확인된 작동 URL)
    items = _api_call(
    ...
    for it in items:
        ...
        try:
            ...
            """, (
                bas_dt, code,
                it.get("itmsNm", ""),
                _fval(it.get("srtSelVol") or it.get("lendVol")),
                _fval(it.get("srtSelAmt") or it.get("lendAmt")),
                _fval(it.get("stlnRmndrVol") or it.get("lendBalVol")),
                _fval(it.get("stlnRmndrAmt") or it.get("lendBalAmt")),
                _fval(it.get("stlnRmndrRate") or it.get("lendBalRt")),
            ))
            saved += 1
        except Exception as e:
...
def collect_investor_trading(conn, bas_dt: str) -> int:
    ...
    # 신청 서비스에 투자자매매 API가 없으면 0 반환
    items = _api_call(
        f"{BASE_URL}/getInvstByTrdrStkInfo",
        {"basDt": bas_dt},
    )
    if not items:
        logger.debug(f"[투자자] {bas_dt} 데이터 없음 (미신청 서비스)")
        return 0
...
def collect_foreign_holding(conn, bas_dt: str) -> int:
    ...
    if not items:
        logger.debug(f"[외국인] {bas_dt} 데이터 없음 (미신청 서비스)")
        return 0
