...
SUMMARIES_DIR = Path('/Applications/stock_dashboard/reports/deepseek_summaries')
TARGET_STOCKS = [
...
def deepseek_summarize(stock_name: str, file_name: str, context: str) -> str:
    api_key = os.getenv('DEEPSEEK_API_KEY', '').strip()
    if not api_key:
        return (
            f"# {stock_name} 상세분석 (자동요약)\n\n"
            f"- 파일: {file_name}\n"
            "- DEEPSEEK_API_KEY 미설정으로 로컬 요약 사용\n\n"
            f"## 추출 요약\n{context}\n"
        )
    ...
    payload = {
        'model': model,
        'messages': [
            {'role': 'system', 'content': '당신은 재무분석 리포트 작성 보조다.'},
    ...
    try:
        ...
        return f"# {stock_name} 상세분석 (DeepSeek)\n\n- 파일: {file_name}\n\n{txt}\n"
    except Exception as e:
        return (
...
def upsert_post(conn: sqlite3.Connection, fm: FileMeta, body: str):
    ...
    if row:
        pid = row['id']
        conn.execute(
            "UPDATE detailed_analysis_posts SET stock_code=?, content_md=?, source='deepseek_auto', updated_at=datetime('now') WHERE id=?",
            (stock_code, body, pid),
        )
        conn.execute('DELETE FROM detailed_analysis_files WHERE post_id=?', (pid,))
    else:
        cur = conn.execute(
            "INSERT INTO detailed_analysis_posts(stock_code,stock_name,title,content_md,source) VALUES (?,?,?,?, 'deepseek_auto')",
    ...
    # DeepSeek 분석 결과를 별도 md 파일로 저장해 UI에서 다운로드 가능하게 노출
    ...
    summary_name = f"{safe_stock}_{safe_stem}_deepseek.md"
    summary_path = SUMMARIES_DIR / summary_name
    summary_path.write_text(body, encoding='utf-8')
    conn.execute(
        "INSERT INTO detailed_analysis_files(post_id,file_name,file_path,file_type) VALUES (?,?,?,?)",
        (pid, summary_name, str(summary_path), 'deepseek_md'),
    )
...
def main():
    ...
    for fm in files:
        ...
        body = deepseek_summarize(fm.stock_name, fm.path.name, ctx)
