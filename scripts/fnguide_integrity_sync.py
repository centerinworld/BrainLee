...
BACKUP_RETENTION_DAYS = 7  # 최근 N일치만 보관 — 이 잡이 매일 실행되므로 무제한 누적 방지


def prune_old_backups(retention_days=BACKUP_RETENTION_DAYS):
    """financial_data_backup_fnguide_sync_YYYYMMDD 중 retention_days보다 오래된 테이블 삭제.
    2026-08-24: 매일 1회씩 생성되는데 삭제 로직이 없어 76개(0.62GB)까지 무제한 누적된 것을 발견,
    재발방지로 매 실행 시 오래된 백업을 자동 정리한다."""
    from datetime import timedelta
    cutoff = (datetime.now() - timedelta(days=retention_days)).strftime("%Y%m%d")
    rows = conn.execute(
        "SELECT name FROM sqlite_master WHERE type='table' "
        "AND name LIKE 'financial_data_backup_fnguide_sync_________'"
    ).fetchall()
    dropped = 0
    for (name,) in rows:
        date_part = name.replace("financial_data_backup_fnguide_sync_", "")
        if len(date_part) == 8 and date_part.isdigit() and date_part < cutoff:
            conn.execute(f"DROP TABLE IF EXISTS {name}")
            dropped += 1
    if dropped:
        conn.commit()
        print(f"[BACKUP_PRUNE] {retention_days}일 초과 백업 {dropped}개 삭제")
...
for a in mislabeled:
    ...
    for year in years:
        ...
        """, (code, year)).fetchone()
        db_row = conn.execute("""
            SELECT id, report_type, revenue FROM financial_data
            WHERE stock_code=? AND year=? AND is_annual=1
            ORDER BY (CASE WHEN report_type='CFS' THEN 0 ELSE 1 END) ASC LIMIT 1
        """, (code, year)).fetchone()
