...
def _send_telegram_reports(reports: list, now_str: str) -> None:
    ...
    # 기본 정책: 텔레그램은 신규 편입/편출 변화만 보낸다.
    # 전체 분석/보유 목록은 DB에는 저장하지만 매일 같은 종목을 반복 발송하지 않는다.
    if os.getenv("STOCKEASY_TELEGRAM_FULL_REPORT", "0").strip() not in {"1", "true", "TRUE", "yes"}:
        changed = _send_change_reports(reports, now_str)
        if changed == 0:
            logger.info("[스탁이지 텔레그램] 신규 편입/편출 없음 — 발송 생략")
        return
